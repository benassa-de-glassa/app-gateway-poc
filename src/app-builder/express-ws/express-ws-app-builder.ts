...
import { ExpressWsHandler } from './express-ws-adapter';
import { ExpressWsRouterFactory, WsEndpoints, WsRoute } from './express-ws-router-factory';

interface EndpointCollection {
  [path: string]: { endpoints: WsEndpoints[]; middleware: ExpressWsHandler[] };
}

export class ExpressWsAppBuilder {
  private readonly endpoints: EndpointCollection = {};
  private readonly routerFactory = new ExpressWsRouterFactory();
  ...
  public withEndpoint(path: string, endpoints: WsEndpoints, middleware: ExpressHandler[]): ExpressWsAppBuilder {
  ...
  private setupRoute(app: express.Application, route: WsRoute): void {
